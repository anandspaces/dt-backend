...
5. **Async Processing**
   ...
   - **Parse export** (preferred `POST /api/v1/parse/exports` → **202** + `_links`; legacy `POST /api/v1/parse/pdf-export` → **200** with `Deprecation` header): writes a storage manifest and `progress.json`, enqueues atom/topic/chapter generation (TTS via SuperTTS HTTP only when `SUPERTTS_HTTP_URL` is set; otherwise TTS cells are skipped for parse-export; per-atom `lang` `en`|`hi` from script density drives SuperTTS language). Poll **`GET /api/v1/parse/exports/{exportId}/status`** (lightweight) or **`GET /api/v1/parse/export/{exportId}/generated`** for full artifacts; **`GET /api/v1/parse/exports/{exportId}/events`** is SSE when `REDIS_URL` or in-process fallback.
